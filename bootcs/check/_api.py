...
def include(*paths):
    """
    Copy files/directories from the check directory (:data:`bootcs.check.internal.check_dir`),
    ...
    Example usage::
        ...
        bootcs.include("foo.txt", "bar.txt")
...
def hash(file):
    ...
    :type file: str
    :rtype: str
    :raises bootcs.Failure: if ``file`` does not exist
...
def exists(*paths):
    ...
    :raises bootcs.Failure: if any ``path in paths`` does not exist

    Example usage::

        bootcs.exists("foo.c", "foo.h")
...
def import_checks(path):
    ...
    :raises FileNotFoundError: if ``path / .cs50.yaml`` does not exist
    :raises yaml.YAMLError: if ``path / .cs50.yaml`` is not a valid YAML file
    ...
    "less comfy" checks like so::
        ...
        less = bootcs.import_checks("../less")
        from less import *
...
class run:
    ...
    By default, the command will be run using the same environment as ``bootcs``,
    these mappings may be overridden via the ``env`` parameter::

        bootcs.run("./foo").stdin("foo").stdout("bar").exit(0)
        bootcs.run("./foo", env={ "HOME": "/" }).stdin("foo").stdout("bar").exit(0)
    ...
    def __init__(self, command, env={}):
        ...
        # Workaround for OSX pexpect bug
    ...
    def stdout(self, output=None, str_output=None, regex=True, timeout=3, show_timeout=False):
        """
        Retrieve all output from stdout until timeout.
        """
        if output is None:
            self._wait(timeout)
        ...
        try:
            ...
            raise Failure(_("bootcs could not verify output"))
    ...
    def reject(self, timeout=1):
        """
        Check that the process survives for timeout.
        """
